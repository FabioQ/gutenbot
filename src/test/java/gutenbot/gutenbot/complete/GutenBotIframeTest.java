...
public class GutenBotIframeTest {
					...
					for (Object o : f.getSyndEntryList()) {
						...
						if (entry.getPublishedDate().after(maxDate)) {
							...
							if (articleContent.getArticleContent().length() > 20) {
								...
								// and use
								// it in the article.
								int wordsToExtract = 50;
								StringTokenizer st = new StringTokenizer(articleContent.getArticleContent().toString());
							     for(int i = 0; i < wordsToExtract; i++) {
							    	 if (st.hasMoreTokens()) excerptContent = excerptContent + " " + st.nextToken();  
							     }
							     excerptContent += " ... Read More.<br><br>";
							     //adding iFrame
							     excerptContent += "<iframe width=\"615px\" height=\"900px\" src=\"" + entry.getLink()  + "\"></iframe>";
							     articleContent.setArticleContent(excerptContent);
								...
								if (articleEnclosuresList.size() > 0){
									articleContent = parser.addEnclosure(articleContent, articleEnclosuresList.get(0).getUrl());
									articleContent.setArticlePhotoURL(articleEnclosuresList.get(0).getUrl());
								}
