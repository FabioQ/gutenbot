...
public class GutenBotTest {
	...
	public void test() throws IllegalArgumentException, FeedException, IOException, URISyntaxException, XmlRpcFault {
		...
		for (String feedUrl : configuration.getFeedURLs()) {
			...
			String blogCategory = "Calcio"; // TODO this category should be part
											// of feed detail to split feeds
											// between categories.
			...
			for (Object o : f.getSyndEntryList()) {
				...
				if(entry.getPublishedDate().after(maxDate)){
					...
					// if I have any enclosure, it will be set on the article object
					...
					if (articleContent.getArticleContent().length() > 20) {
						// added in the parser a method able to add enclosures, it
						// takes the first enclosure of an article, if any, and use
						// it in the article.
